...
for _email in $_email_files; do 
 ...
 # Mon, 19 Mar 2018 15
  ...
  #if [ $_day -lt 10 ]; then
    #PrintMessage "$_yea$_Mnt_long"0"$_day$_hrs$_min.$_sec" "$_email"
  #  touch -a -m -t "$_yea$_Mnt_long"0"$_day$_hrs$_min.$_sec" "$_email"
  #else
  #  touch -a -m -t "$_yea$_Mnt_long$_day$_hrs$_min.$_sec" "$_email"
  #fi

  PrintMessage ${email/$_pathToEmails/}

  if [ $_day -lt 10 ]; then
    touch -t "$_yea$_Mnt_long"0"$_day$_hrs$_min.$_sec" "$_pathToStore$_yea/$_Mnt_long/0$_day/$_email"
  else
    touch -t "$_yea$_Mnt_long$_day$_hrs$_min.$_sec" "$_pathToStore$_yea/$_Mnt_long/$_day/$_email"
  fi
